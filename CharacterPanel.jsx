...
(function (thisObj) {
    ...
    // ================= 分鏡 / 運鏡 / 預覽效能 =================
    //
    // 這個專案的剪輯結構:105期 裡每一鏡 = 一整疊圖層,全部 parent 到一顆「BG」(鏡頭層),
    // 運鏡(推拉搖)的 Position/Scale key 打在那顆鏡頭層上。下面工具就是針對這個工法。
    ...
    // 運鏡:在目前時間,對選取的鏡頭層(BG/Null)把現在的 Position+Scale 記成一個 key。
    ...
    function cutNextShot() {
        ...
        // 下一個 comp 標記(在 shotEnd 之後);沒有就沿用本鏡長度
        var nextMark = null;
        try {
        ...
        showStatus("已切下一鏡:複製 " + sel.length + " 個圖層,放到 " +
            newStart.toFixed(2) + "s ~ " + newEnd.toFixed(2) + "s" +
            (nextMark !== null ? "(對齊下個標記)" : "(無標記,沿用本鏡長度)") +
            "。新鏡已選起,直接運鏡即可。");
    }

    // 每鏡單獨預覽:把工作區框到「選取圖層的範圍」(沒選就用目前鏡頭附近),方便只預覽這一鏡。
    function shotToWorkArea() {
        ...
        } else { alert("先選取這一鏡的圖層,再按(用來框工作區)。"); return; }
    ...
    // 預覽解析度切換(Full / Half / Third):降解析度讓播放變順
    ...
    // 編輯時關特效:把目前合成裡的 Drop Shadow 效果 + 調整圖層暫時關掉,讓對畫面變順。
    // on=false 關、on=true 開回來(會把全部陰影/調整層都打開)。
        ...
        try {
            for (var i = 1; i <= comp.numLayers; i++) {
                ...
                for (var f = 1; f <= fx.numProperties; f++) {
                    ...
                    // 吃效能的模糊/陰影類:Drop Shadow、Gaussian/Fast Blur
                    var nm = (ef.matchName || "") + " " + (ef.name || "");
                    if (/Drop Shadow|Gaussian Blur|Fast Blur|ADBE Box Blur/i.test(nm)) {
    ...
    // 靜圖代理:把選取圖層的「來源合成」算成一張 PNG 當 proxy,預覽時用輕量靜圖、輸出時可切回。
    // 適合「不會動的大插圖 precomp」;若 precomp 內有動畫(眨眼/嘴),套了會凍結,請勿對它用。
    function makeStillProxy() {
        var comp = activeComp(); if (!comp) return;
        var sel = comp.selectedLayers;
        if (sel.length === 0) { alert("先選取要做代理的圖層(來源是合成的大插圖),再按。"); return; }

        // proxy 輸出資料夾:專案旁的 _proxy,沒存檔就丟系統暫存
        var dir;
        try {
        ...
        try {
            for (var i = 0; i < sel.length; i++) {
                ...
                try {
                    ...
                    src.saveFrameToPng(src.time, f);   // 算來源合成目前這一格成 PNG
                    src.setProxy(f);                   // 指派為 proxy(之後可用 useProxy 開關)
                    done++;
    ...
    function toggleProxyUse(use) {
        ...
        for (var i = 0; i < sel.length; i++) {
            var src = sel[i].source;
            try { if (src && src.useProxy !== undefined && src.hasProxy) { src.useProxy = use; n++; } } catch (e) {}
        }
    ...
    function buildUI(thisObj) {
        ...
        // ── 運鏡(兩擊框鏡頭,key 打在選取的 BG/Null 鏡頭層)──
        p7.add("statictext", undefined, "運鏡:選鏡頭層(BG/Null)→開頭框好按起幀→結尾框好按迄幀:");
        var rowCam = p7.add("group");
        var bCamIn  = rowCam.add("button", undefined, "設起幀");   bCamIn.preferredSize.width = 70;
        var bCamOut = rowCam.add("button", undefined, "設迄幀+緩動"); bCamOut.preferredSize.width = 100;
        bCamIn.onClick  = function () { camSetKey(false); };
        bCamOut.onClick = function () { camSetKey(true); };
        var rowCam2 = p7.add("group");
        ...
        // ── 切下一鏡 ──
        var rowCut = p7.add("group");
        rowCut.add("statictext", undefined, "切鏡:").preferredSize.width = 56;
        var bCut = rowCut.add("button", undefined, "切下一鏡(複製整疊→對齊下個標記)"); bCut.preferredSize.width = 240;
        bCut.onClick = cutNextShot;

        // ── 預覽效能 ──
        p7.add("statictext", undefined, "─ 預覽效能(對畫面卡的時候用)─");
        var rowFx = p7.add("group");
        rowFx.add("statictext", undefined, "特效:").preferredSize.width = 56;
        var bFxOff = rowFx.add("button", undefined, "編輯模式(關陰影/模糊)"); bFxOff.preferredSize.width = 150;
        ...
        var rowWA = p7.add("group");
        rowWA.add("statictext", undefined, "預覽:").preferredSize.width = 56;
        var bWA = rowWA.add("button", undefined, "工作區框到本鏡(選取圖層範圍)"); bWA.preferredSize.width = 220;
        bWA.onClick = shotToWorkArea;

        var rowPx = p7.add("group");
        ...
        var rowPx2 = p7.add("group");
        rowPx2.add("statictext", undefined, "").preferredSize.width = 56;
        rowPx2.add("statictext", undefined, "只給「不會動的大插圖」用;會動的(眨眼/嘴)套了會凍結");
